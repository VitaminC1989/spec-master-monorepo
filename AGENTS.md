...
This pnpm + Turbo monorepo keeps runnable apps under `apps/`. `apps/api` houses the NestJS service, Prisma schema, and unit specs beside their sources (`src/**/*.spec.ts`), while e2e suites live in `apps/api/test/`. `apps/web` contains the Vite + React client plus mock handlers in `apps/web/api/` for local uploads. Shared documentation sits in `docs/`, and `docker-compose.yml` provisions Postgres for migrations and automated tests.

## Build, Test, and Development Commands
Install dependencies once with `pnpm install`. Use `pnpm dev` for concurrent API and web watchers. Scope work with `pnpm --filter @spec/api dev` or `pnpm --filter @spec/web dev`. Back-end builds with `pnpm --filter @spec/api build`; the front-end runs `pnpm --filter @spec/web build:check` for type-checking and bundling. Start Postgres via `docker compose up db`. Run unit tests using `pnpm --filter @spec/api test`, coverage with `test:cov`, e2e flows with `test:e2e`, and front-end linting through `pnpm --filter @spec/web lint`.

## Coding Style & Naming Conventions
Both apps are TypeScript-first. Prettier enforces two-space indentation, single quotes, and trailing commas; run `pnpm --filter @spec/api format` when needed. ESLint extends `typescript-eslint` and `eslint-plugin-prettier`. File naming favors PascalCase for React components and Nest classes, camelCase for hooks or utilities, and explicit `*Dto`/`*Entity` suffixes. Keep environment variables in `.env` (API) or `.env.local` (web) using `SCREAMING_SNAKE_CASE` keys.

## Testing Guidelines
Use Jest for API tests: colocate unit specs and reserve `apps/api/test/` for e2e scenarios configured through `test/jest-e2e.json`. Boot the Postgres container or equivalent when running migrations, seeds, or e2e suites, and reset data between runs. Target `pnpm --filter @spec/api test:cov` before PRs. For UI work, add React Testing Library or Playwright tests under `apps/web` and wire them into `pnpm --filter @spec/web lint` or a future `test` script.

## Commit & Pull Request Guidelines
Follow Conventional Commits (`feat:`, `fix:`, `chore:`) with imperative summaries and reference issue IDs when applicable. Each PR should explain scope, list migrations or env additions, note executed commands, and attach screenshots or curl samples for visible or API changes. Request at least one review, keep branches focused, and update related docs under `docs/` or localized folders when behavior shifts.

## Security & Configuration Tips
Never commit credentials; rely on `.env.example` placeholders. Run `pnpm --filter @spec/api prisma migrate dev` after schema changes so generated clients stay in sync with Postgres. Use `apps/web/scripts/local-qiniu-server.js` for local upload tokens instead of reaching for production keys.
